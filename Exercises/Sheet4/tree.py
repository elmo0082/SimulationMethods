...
class TreeClass:
    ...
    def gforce(self,nodeid,pid,anglemax):
        ...
        #
        if self.nodelist[nodeid].mass > 0:
            if angle < anglemax or self.nodelist[nodeid].leaf == 1:
                ...
                if distance != 0:
                    force[0] = self.nodelist[nodeid].mass * \
                               np.power(distance + 0.001**2, -3/2) \
                                * (self.particles[pid][0] - self.nodelist[nodeid].cm[0])
                    force[1] = self.nodelist[nodeid].mass * \
                               np.power(distance + 0.001**2, -3/2) \
                               * (self.particles[pid][1] - self.nodelist[nodeid].cm[1])
                    force[2] = self.nodelist[nodeid].mass * \
                               np.power(distance + 0.001**2, -3/2) \
                               * (self.particles[pid][2] - self.nodelist[nodeid].cm[2])
            else:
                #
                ...
                for iz in range(0,2):
                    for iy in range(0,2):
                        for ix in range(0,2):
                            ...
                            cforce, cinteractions = self.gforce(cid,pid,anglemax)
                            force[:] = force + cforce
                            interactions = interactions + cinteractions
            #
    ...
    def allgforces(self, anglemax):
        ...
        for pid in range(nrp):
            gforce, interaction = self.gforce(0, pid, anglemax)
            self.forces[pid][:] = gforce
            interactions = interactions + interaction
        print(interactions/nrp)

    def exact_force(self, pid):
        force = [0., 0., 0.]
        # Calculate force on particle pid with all other particles id
